...
if [ $MT_DNN == "TRUE" ]
then
    OUTPUT_DIR=/tmp/mt_dnn_models/bert-large/$TASK/$LAYER_FOLDER_NAME/$SEED
    LOG_FILE_DIR=mt_dnn_logs/bert-large/$TASK/
else
    OUTPUT_DIR=/tmp/models/bert-large/$TASK/$LAYER_FOLDER_NAME/$SEED
...
then
    ...
    if [ $MT_DNN == "TRUE" ]
    then
        MM_OUTPUT_DIR=/tmp/mt_dnn_models/bert-large/$TASK-MM/$LAYER_FOLDER_NAME/$SEED
        MM_LOG_FILE_DIR=mt_dnn_logs/bert-large/$TASK-MM/
    else
        MM_OUTPUT_DIR=/tmp/models/bert-large/$TASK-MM/$LAYER_FOLDER_NAME/$SEED
...
then
    ...
    then
        python examples/run_glue.py \
          ...
          --mt_model_path /trained_model/mt-dnn-bert/large.pt \
          --task_name $TASK \
          --do_train \
          --do_eval \
          --do_lower_case \
